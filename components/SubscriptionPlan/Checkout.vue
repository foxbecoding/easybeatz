<template>
    <div class="pa-4">
        <form id="payment-form">
            ...
            >
                <span class="text-background">Subscribe</span>
            </button>
...
const loadStripeModal = (): void => {
    const client_secret = subscriptionPlanStore.client_secret
    loadStripe(config.public.STRIPE_PUBLISHABLE_KEY).then(res=>{
        ...
        subscribe_user(elements, stripe)
    })          
}

const subscribe_user = (elements: StripeElements, stripe: Stripe): void =>{
    ...
    form.addEventListener('submit', async (event: any) => {
        ...
        const {error} = await stripe.confirmPayment({
            ...
            elements,
            confirmParams: {
                return_url: `${base_url}/subscription-plan/confirmation/${subscriptionPlanStore.pricePackage_pk}`,
            }
        });
